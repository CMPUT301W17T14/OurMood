...
public class SeeMapActivity extends FragmentActivity implements OnMapReadyCallback {
     ...
     */
    @Override
    public void onMapReady(GoogleMap googleMap) { // get permission and get map
        mMap = googleMap;
        ...
        int caller = getIntent().getIntExtra("caller", 0); // get caller info
        ArrayList<Mood> mMoodList;

        switch (caller) {
            case 1: // show emotional status
                mMoodList = MoodeventFragment.filteredMoodList;
                Collections.reverse(mMoodList);
                for (Mood mood : mMoodList) { // get all moods
                    if (mood.getLocation() != null ) {
                        ...
                        if (moodStatus.equals("angry")) {
                            ...
                            googleMap.addMarker(new MarkerOptions().icon(BitmapDescriptorFactory.fromResource(R.drawable.timg))
                                    .position(new LatLng(mood.getLocation().getLatitude(),
                                            mood.getLocation().getLongitude())));
                ...
                break;
            case 2: // show name and emotional status
