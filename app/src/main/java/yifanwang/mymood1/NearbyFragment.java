...
public class NearbyFragment extends Fragment implements OnMapReadyCallback,
    ...
    public void onHiddenChanged (boolean hidden) { // when this fragment is becoming visible
    ...
    @Override
    public void onMapReady(GoogleMap googleMap) { // get permission and show map
        myMap = googleMap;
    ...
    private void show() { // setting marker
        if (ContextCompat.checkSelfPermission(this.getActivity(), Manifest.permission.ACCESS_FINE_LOCATION)
                != PackageManager.PERMISSION_GRANTED) {
            return;
        }
        currentLocation = LocationServices.FusedLocationApi.getLastLocation( // get current location
                mGoogleApiClient);
            ...
            try {
                ...
                for (User user : userlist) {
                    ...
                    for (Mood mood : moodList) {
                        // get all moods within 5 km and has location info
                        if (mood.getLocation() != null){
                            if (currentLocation.distanceTo(mood.getLocation()) < 5000) {
                                ...
                                if (moodStatus.equals("angry")) {
                                    ...
                                    myMap.addMarker(new MarkerOptions().icon(BitmapDescriptorFactory.fromResource(R.drawable.timg))
                                            .position(new LatLng(mood.getLocation().getLatitude(),
                                                    mood.getLocation().getLongitude())));
